...
The current version of the package is 0.1 and was published on 22.06.2023.
...
head(loggerinfo, n = 3) %>% 
  kable() %>%
  kable_styling(font_size = 13, html_font = "arial", bootstrap_options = "striped")
```
...
head(loggerdata, n = 10) %>% 
  kable() %>%
  kable_styling(font_size = 10, html_font = "arial", bootstrap_options = "striped")
```
